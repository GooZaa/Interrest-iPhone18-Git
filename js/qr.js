...
function qrLib(typeNumber, ecc) {
  var el = { L: 1, M: 0, Q: 3, H: 2 }[ecc] || 0;
  return {
    addData: function (s) { this._data = s; },
    make: function () {
      var t = typeNumber;
      if (!t) {
        t = 4; // default version suitable for typical URLs
      }
      this._m = new QRCodeModel(t, el);
      this._m.addData(this._data);
      this._m.make();
    },
    getModuleCount: function () { return this._m.getModuleCount(); },
    isDark: function (r, c) { return this._m.isDark(r, c); }
  };
}

function qrSvgMarkup(text, size) {
  var qr = qrLib(0, 'M');
  qr.addData(String(text || ''));
  qr.make();
  var count = qr.getModuleCount(), quiet = 4, total = count + (quiet * 2), path = [];
  for (var row = 0; row < count; row++) {
    for (var col = 0; col < count; col++) {
      if (qr.isDark(row, col)) path.push('M' + (col + quiet) + ' ' + (row + quiet) + 'h1v1h-1z');
    }
  }
  var px = Math.max(64, Number(size) || 132);
  return '<svg xmlns="http://www.w3.org/2000/svg" width="' + px + '" height="' + px + '" viewBox="0 0 ' + total + ' ' + total + '" role="presentation" shape-rendering="crispEdges"><rect width="100%" height="100%" fill="#fff"/><path d="' + path.join('') + '" fill="#0F172A"/></svg>';
}
